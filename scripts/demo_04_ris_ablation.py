...
from restir_gs.eval.ris_ablation import run_ris_ablation, summarize_rows
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run RIS ablation sweeps over K and seeds.")
    ...
    rows = run_ris_ablation(gbuffer, lights, args.k_values, args.seed_count)
    summary = summarize_rows(rows)

    args.output_dir.mkdir(parents=True, exist_ok=True)
    csv_path = args.output_dir / "ris_ablation.csv"
    json_path = args.output_dir / "ris_ablation_summary.json"
    ...
    payload = {
        "metadata": {
            ...
            "candidate_seed_base": 3100,
            "selection_seed_base": 4100,
            "row_count": len(rows),
        },
        "summary": summary,
    }
    ...
    print("quantity, estimator, K, samples, MAE mean/std, RMSE mean/std")
    for row in summary:
        print(
            f"{row['reference_quantity']}, {row['estimator']}, K={row['k']}, "
