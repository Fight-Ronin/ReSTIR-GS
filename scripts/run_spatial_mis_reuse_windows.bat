...
if not defined RESTIRGS_PLY (
  echo Missing RESTIRGS_PLY. Set it to a real 3DGS point_cloud.ply path, for example:
  echo   set "RESTIRGS_PLY=C:\path\to\point_cloud.ply"
  exit /b 1
)

if not defined RESTIRGS_CAMERA_CONFIG (
  echo Missing RESTIRGS_CAMERA_CONFIG. Run scripts\run_camera_probe_windows.bat first, then set:
  echo   set "RESTIRGS_CAMERA_CONFIG=outputs\camera_probe_selected_camera.json"
  exit /b 1
)
...
if not defined MPLCONFIGDIR set "MPLCONFIGDIR=C:\tmp\matplotlib_restirgs"
...
"%RESTIRGS_ENV%\python.exe" scripts\demo_14_spatial_mis_reuse.py --ply "%RESTIRGS_PLY%" --camera-config "%RESTIRGS_CAMERA_CONFIG%" --num-lights 128 --candidate-count 8 --candidate-seed 14100 --initial-selection-seed 14200 --max-gaussians 200000 --output-dir outputs
